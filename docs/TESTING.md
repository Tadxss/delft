...
| `data-export.spec.ts`         | Account modal → "Export my data" downloads a single JSON with every RLS-visible workspace's pages/canvases/folders/credentials; credential secrets are present in encrypted form (`secretCiphertext`/`secretIv`/`vaultSalt`) always, and decrypted inline only when the vault is unlocked at export time.                                                                                                                                                                                                                                                                                                                                                 |
| `ownership-transfer.spec.ts`  | Owner A "Make owner" on member B in the Members modal → B becomes `owner`, A becomes `editor`, invalidations refresh A's own owner-gated UI; blocked with a clear message while the workspace has a credentials vault.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
